...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WorkArena L1                 | Strongest first target: routine forms, lists, menus, knowledge base, and catalog interactions align with the Fast Path skills. | A fail-closed semantic-BID adapter and pinned full-suite runner now cover explicit search, fill, select, safe navigation clicks, bounded `then` plans, and the benchmark's knowledge-base, menu-navigation, and field-fill templates. It stops on observed action errors or a missing BrowserGym action receipt rather than continuing from an unverified state; mutation approval, broad task planning, richer recovery, gated ServiceNow access, and external execution are still required. |
| WorkArena L2/L3              | Good long-horizon product direction.                                                                                           | The adapter can advance explicit low-risk `then` plans, but it does not yet perform task decomposition, post-action verification, or recovery across composite workflows.                                                                                                                                                                                                                                                                                                                     |
...
- [BrowserGym](https://github.com/ServiceNow/BrowserGym) exposes the benchmark action loop separately from a browser agent's internal Playwright executor. LHIC now includes a deliberately limited, fail-closed semantic-BID adapter for explicit search, fill, select, and safe clicks, but a local Playwright test alone is not a valid external benchmark run.
