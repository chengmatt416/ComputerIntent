...
- `directSemantic`: LHIC's label, role, name, and placeholder-aware `fillForm` skill, with DOM verification that the primary field retained its value. It does not submit the fixture.
...
Both treatments operate on fresh copies of the same rendered task and must retain the same requested input value. The report calls a gap of at least 50 percentage points, with semantic success at least 95%, an `observedLargeControlledAdvantage`.
